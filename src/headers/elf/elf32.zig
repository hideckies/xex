...
pub const ELFProgramHeader32 = struct {
    p_type: u32,
    p_offset: u32,
    p_vaddr: u32,
    p_paddr: u32,
...
pub const ELFSectionHeader32 = struct {
    sh_name: u32,
    sh_type: u32,
    sh_flags: u32,
    sh_addr: u32,
    ...
    pub fn format(
        ...
        return writer.print(str, .{
            ...
            try cham.magenta().fmt("{s}", .{(try decode.HAlign.parse(self.sh_addralign)).str}),
        });
    }
};
...
pub const ELF32_Sym = struct {
    ...
    pub fn format(
        ...
        return writer.print(str, .{
            ...
            try cham.cyanBright().fmt("0x{x:0>8}", .{self.st_value}),
            try cham.cyanBright().fmt("0x{x:0>8}", .{self.st_size}),
...
pub const ELFHeaders32 = struct {
    allocator: std.mem.Allocator,
    file_path: []const u8,
    file_header: ELFHeader32,
    program_headers: []ELFProgramHeader32,
    section_headers: []ELFSectionHeader32,
    symbols: []ELF32_Sym,
    dynsymbols: []ELF32_Sym,
    ...
    pub fn printInfo(self: Self) !void {
        ...
        , .{
            ...
            "Start Address",
            try cham.greenBright().fmt("0x{x:0>8}", .{self.file_header.e_entry}),
        });
        try stdout.print("{s}\n", .{str});
    }
    ...
    pub fn analyze(allocator: std.mem.Allocator, file_path: []const u8, reader: anytype) !Self {
        ...
        const elf_header32 = ELFHeader32{
            .e_ident = e_ident,
            .e_type = try reader.readInt(u16, endian),
            .e_machine = try reader.readInt(u16, endian),
            .e_version = try reader.readInt(u32, endian),
            .e_entry = try reader.readInt(u32, endian),
            .e_phoff = try reader.readInt(u32, endian),
            .e_shoff = try reader.readInt(u32, endian),
        ...
        // const object_type = try decode.ObjectType.parse(elf_header32.e_type);

        try reader.context.seekTo(elf_header32.e_phoff);

        var program_headers = std.ArrayList(ELFProgramHeader32).init(allocator);
        defer program_headers.deinit();
        for (0..elf_header32.e_phnum) |_| {
            const ph = ELFProgramHeader32{
                .p_type = try reader.readInt(u32, endian),
                .p_offset = try reader.readInt(u32, endian),
                .p_vaddr = try reader.readInt(u32, endian),
                .p_paddr = try reader.readInt(u32, endian),
                .p_filesz = try reader.readInt(u32, endian),
                .p_memsz = try reader.readInt(u32, endian),
                .p_flags = try reader.readInt(u32, endian),
                .p_align = try reader.readInt(u32, endian),
            };
            try program_headers.append(ph);
        }

        try reader.context.seekTo(elf_header32.e_shoff);

        var section_headers = std.ArrayList(ELFSectionHeader32).init(allocator);
        defer section_headers.deinit();
        for (0..elf_header32.e_shnum) |_| {
            const sh = ELFSectionHeader32{
                .sh_name = try reader.readInt(u32, endian),
                .sh_type = try reader.readInt(u32, endian),
                .sh_flags = try reader.readInt(u32, endian),
                .sh_addr = try reader.readInt(u32, endian),
                .sh_offset = try reader.readInt(u32, endian),
                .sh_size = try reader.readInt(u32, endian),
                .sh_link = try reader.readInt(u32, endian),
                .sh_info = try reader.readInt(u32, endian),
                .sh_addralign = try reader.readInt(u32, endian),
                .sh_entsize = try reader.readInt(u32, endian),
        ...
        const shstrtab_hdr = section_headers.items[elf_header32.e_shstrndx];
        ...
        for (section_headers.items) |*sh| {
            ...
            if (section_name_end) |end_idx| {
                ...
                const section_name = section_name_slice[0 .. end_idx + 1]; // add null-terminated character.
                sh.sh_name_str = section_name;
            }
        }

        // Find symbol table from section headers, and get symbols.
        var symtab_hdr: ?ELFSectionHeader32 = null;
        var strtab_hdr: ?ELFSectionHeader32 = null;
        var dynsym_hdr: ?ELFSectionHeader32 = null;
        var dynstr_hdr: ?ELFSectionHeader32 = null;
        for (section_headers.items) |sh| {
        ...
        // Get symbols
        const real_sym_size: usize = @sizeOf(u8) * 2 + @sizeOf(u16) + @sizeOf(u32) * 3;
        var symbols = std.ArrayList(ELF32_Sym).init(allocator);
        ...
        if (symtab_hdr != null) {
            ...
            for (num_symbols) |_| {
                ...
                var sym = ELF32_Sym{
                    .st_name = st_name,
                    .st_value = try reader.readInt(u32, endian),
                    .st_size = try reader.readInt(u32, endian),
        ...
        // Get dynamic symbols
        var dynsymbols = std.ArrayList(ELF32_Sym).init(allocator);
