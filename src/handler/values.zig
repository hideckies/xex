...
pub fn print(dbg: *Debugger, cmd: Command) !void {
    ...
    if (std.mem.startsWith(u8, target, "0x")) {
        ...
        const value = ptrace.readRegister(dbg.process.pid, target) catch |err| {
            return stdout.print_error(dbg.allocator, "error: {}\n", .{err});
        };
...
pub fn prints(dbg: *Debugger, cmd: Command) !void {
    ...
    if (std.mem.startsWith(u8, target, "0x")) {
        ...
        addr = ptrace.readRegister(dbg.process.pid, target) catch |err| {
            return stdout.print_error(dbg.allocator, "error: {}\n", .{err});
        };
    }
...
pub fn set(dbg: *Debugger, cmd: Command) !void {
    ...
    if (std.mem.eql(u8, target[0..1], "0x")) {
        ...
        ptrace.writeRegister(dbg.process.pid, target, value_num) catch |err| {
            return stdout.print_error(dbg.allocator, "error: {}\n", .{err});
        };
    }
}
